...
export const create = ({ bodymen: { body: { email, link } } }, res, next) =>
  User.findOne({ email })
    ...
    .then((reset) => {
      ...
      const content = `
        Hey, ${user.name}.<br><br>
        You requested a new password for your asthmaSOSServer account.<br>
        ...
        &mdash; asthmaSOSServer Team
      `
      return sendMail({ toEmail: email, subject: 'asthmaSOSServer - Password Reset', content })
    })
